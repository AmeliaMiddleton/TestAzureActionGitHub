...
- **App Service Plan**: Creates a Linux plan with the selected tier and specifications (`--is-linux`)
- **Web App**: Creates a Linux-based web app with .NET Core 8.0 runtime

#### 4. Web App Configuration
- **Startup Command**: Sets `dotnet TestAzureActionGithub.dll` as the startup file
- **Runtime**: Automatically configured as .NET Core 8.0 on Linux via `--runtime "DOTNETCORE:8.0"`
...
- **Current Approach**: Creates Linux web app with `--runtime "DOTNETCORE:8.0"` for proper .NET Core 8.0 configuration
...
- **Runtime Issues**: Verify .NET Core 8.0 is properly configured via Azure portal (Linux runtime should be "DOTNETCORE:8.0")
